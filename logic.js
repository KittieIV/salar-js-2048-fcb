...
// If one of these variables became true, it means tha player already won
...
function setGame() {
	...
	// Backend board
	// Goal, we will use the backend board to design and move the tiles of the frontent board.

	// Loops are code ti repeat tasks inside it, until it fullfill its task
...
function handleSlide(e){
	...
	if(hasLost()){
		...
		setTimeout(()=>{
			alert("Game Over! Ok lang yan, wag kang susuko. Ipaglaban mo siya!");
			restartGame();
...
function slideLeft(){
	...
	for(let r=0; r<rows; r++){
		...
		// Beacuse of this loop, we are able to update the ids and color of all tiles
		for(let c=0; c<columns; c++){
			let tile = document.getElementById(r.toString() +"-"+ c.toString());
			let num = board[r][c];
			updateTile(tile, num); // update using updateTile function
